...
{
	std::vector< demand_t * > alien_pointers;
	alien_pointers.reserve( vector_size );
	...
	for( std::size_t c = 0; c != cycles_count; ++c )
	{
		...
		alien_pointers.clear();

		while( 0 == alien_clear->m_vector_size.load( std::memory_order_acquire ) )
			std::this_thread::yield();
		for( auto & p : alien_clear->m_demands )
		{
			alien_pointers.push_back( p );
		}
	...
	const char * operation_name )
{
	...
	benchmarker.finish_and_show_stats( 
			thread_count * cycles_count * vector_size, operation_name );
}
