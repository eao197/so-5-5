...
//FIXME: this class could be represented as template class.
//
// disp_binder_t
//
/*!
...
class real_private_dispatcher_t : public private_dispatcher_t
	...
	public :
		...
		real_private_dispatcher_t(
			...
			:	m_disp{ new dispatcher_t{ quotes } }
			{
...
create_disp( const quotes_t & quotes )
	{
		return so_5::rt::dispatcher_unique_ptr_t(
				new impl::dispatcher_t{ quotes } );
	}
	...
	const std::string & data_sources_name_base )
	{
		return private_dispatcher_handle_t{
				new impl::real_private_dispatcher_t{
						env, quotes, data_sources_name_base
				}
			};
	}
