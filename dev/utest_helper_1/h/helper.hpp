...
*/
#if !defined( _UTEST_HELPER_1__HELPER__HELPER_HPP_ )
#define _UTEST_HELPER_1__HELPER__HELPER_HPP_
		...
		public std::runtime_error
{
	...
	public:
		...
		virtual ~check_ex_t() throw()
		{}
};

#define UT_CHECK_THROW__( file, line, exception_class, body )\
do \
{ \
...
UT_CHECK_THROW__(__FILE__, __LINE__, exception_class, body )
...
//! Макрос для облегчения написания юнит теста.
#define UT_UNIT_TEST( test_name ) \
	void test_name ## __impl( ::utest_helper_1::context_stack_t & __test__context ); \
	int test_name() \
	{ \
		::utest_helper_1::context_stack_t __test__context; \
		bool has_exceptions = false; \
		try{ \
			test_name ## __impl ( __test__context ); \
		...
		\
		if( has_exceptions && !__test__context.empty() ) \
		{ \
			std::cerr << "\nContext:\n"; \
			while( !__test__context.empty() ) \
			{ \
				std::cerr << __test__context.top();\
				__test__context.pop(); \
			} \
		} \
...
#define UT_CONTEXT_DECL ::utest_helper_1::context_stack_t & __test__context

//! Передать вызываемой функции контекст
#define UT_CONTEXT __test__context
