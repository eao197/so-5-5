...
void
do_check_drop_newest( so_5::rt::environment_t & env )
{
	do_check_drop_newest_impl(
...
void
do_check_remove_oldest( so_5::rt::environment_t & env )
{
	do_check_remove_oldest_impl(
			env, "dynamic", props::storage_memory::dynamic );
	do_check_remove_oldest_impl(
...
void
do_check_throw_exception( so_5::rt::environment_t & env )
{
	do_check_throw_exception_impl(
			env, "dynamic", props::storage_memory::dynamic );
	do_check_throw_exception_impl(
...
UT_UNIT_TEST( test_no_wait )
{
	run_with_time_limit(
		[] {
			...
			do_check_drop_newest( env.environment() );
			do_check_remove_oldest( env.environment() );
			do_check_throw_exception( env.environment() );
		},
		4,
		"test_no_wait" );
}
