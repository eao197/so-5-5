	...
	a_worker_t(
		...
		:	so_5::rt::agent_t( env,
				tuning_options().message_limits(
					message_limit< msg_request >( 2 ).redirect( self_ptr() ) ) )
...
init( so_5::rt::environment_t & env )
{
	...
	auto w1 = coop->make_agent< a_worker_t >( "[one]" );
	
	coop->make_agent< a_manager_t >( w1->so_direct_mbox(), "[one][one]" );
...
main()
{
	try
	{
		run_with_time_limit(
			...
			2,
			"simple too deep message redirect on MPSC-mboxes test" );
	}
